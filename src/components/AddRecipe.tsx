...
const AddRecipe = ({ open,onClose }: { open: boolean, onClose: () => void }) => {
    ...
    const handleClose = () => { setOpenForm(false);onClose() ;reset()};
    ...
    return (
        <Modal open={openForm}>
            <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={popupStyle}>
                ...
                <Button onClick={() => append('')} variant="outlined"  color="primary" sx={{ mb: 2 ,borderRadius: '50%', minWidth: '40px', minHeight: '40px'}}><Add /></Button>
